  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (submitError instanceof ApiRequestError) {
        ...
        setError(submitError instanceof Error ? submitError.message : "No se ha podido completar la operacion.");
      }
    } finally {
      setLoading(false);
    }
    ...
    <section className="auth-shell">
      <article className="card auth-card">
        ...
        <form className="form-grid" onSubmit={handleSubmit}>
          ...
          {error && Object.keys(fieldErrors).length === 0 ? <p className="feedback error">{error}</p> : null}
